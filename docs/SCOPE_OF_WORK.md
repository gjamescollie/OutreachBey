...
| B5 | Reliability layer | **Auto-restart already done** via `docker-compose` `restart: unless-stopped`. **Uptime monitor in progress:** port 3001 health server added to `index.js`, Docker port exposed in `docker-compose.yml` — pending merge + UFW setup on droplet (`ufw allow ssh`, `ufw allow 3001/tcp`, `ufw enable`). Remaining: daily heartbeat (specced in `FUTURE_PLANS.md`) |
